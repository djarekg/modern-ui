...
export type MixinBase<ExpectedBase = object> = abstract new (
  // Mixins must have a constructor with `...args: unknown[]`
  ...args: unknown[]
) => ExpectedBase;
 ...
 *  base class argument.
 * @template MixinClass Optional interface of fuctionality that was mixed in. Omit if no
 *   additional APIs were added (such as purely overriding base class functionality).
 *
 * @example
 * ```ts
 * interface Foo {

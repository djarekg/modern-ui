...
const DISPLAYED_DURATION = 20_000;
...
export const toast = async (options: ToastOptions) => {
  ...
  const toast = document.createElement('mui-logger-toast');
  toast.type = type;
...
define('mui-logger-toast', Toast, { observedAttributes: ['type'] });

declare global {
  interface HTMLElementTagNameMap {
    'mui-logger-toast': HTMLElement & ToastProps;
  }
}
